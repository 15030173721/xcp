...
function AdPrepare(id, title, url, mode, pic, width, height, top, side, autoClose, hasButton)
{
	if (!AdConfig)
		AdConfigInit();

	var content = AdContent(url, pic, width, height, hasButton, id);
	if (mode == "popup")
	{
...
function AdContent(url, pic, width, height, hasButton, id)
{
	...
	if (picType == "swf")
	{
		...
		if (url != "")
			if (hasButton == 1)
				content = "<span style='z-index:1;position:absolute;'><a href='" + url + "' target='_blank'><img src='js/blank.gif' width='" + width + "' height='" + height + "' /></a><span onclick='closed("+ id +")' style='position:absolute; top:0; right:0;'><img src='js/closed.gif' /></span></span>" + content;
			else if (hasButton == 2)
				content = "<span style='z-index:1;position:absolute;'><a href='" + url + "' target='_blank'><img src='js/blank.gif' width='" + width + "' height='" + height + "' /></a><span onclick='closed("+ id +")' style='position:absolute; top:0; right:0;'>关闭</span></span>" + content;
			else
	...
	else
	{
		...
		if (url != "")
			if (hasButton == 1)
				content = "<a href='" + url + "' target='_blank'>" + content + "</a><span onclick='closed("+ id +")' style='position:absolute; top:0; right:0;'><img src='js/closed.gif' /></span></span>";
			else if (hasButton == 2)
				content = "<span style='z-index:1;position:absolute;'><a href='" + url + "' target='_blank'><img src='js/blank.gif' width='" + width + "' height='" + height + "' /></a><span onclick='closed("+ id +")' style='position:absolute; top:0; right:0;'>关闭</span></span>" + content;
			else
